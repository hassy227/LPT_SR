...
int EXballYim_Y(float Yim,TABLE table){
...
int EXballXim_X(float Xim,int Y,TABLE table){
    return int((Xim/table.Xim)
              *(Y*(table.XT-table.XB)/(table.Yt-table.Yb)
              +(table.XB*table.Yt-table.XT*table.Yb)/(table.Yt-table.Yb))
               +width/2.0);
}
//Y座標からボールの大きさを知る
int EXballBIG(int Y,TABLE table){
...
//卓球台の中央線の情報を入力する
void EXtableCenter(TABLE table,BALL ball){
  table.LY=EXballYim_Y(ball.areaY*0.5,table);//中央線y
